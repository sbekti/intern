...
func TestSessionListPrintsCurrentSessionAndPagination(t *testing.T) {
	...
	if strings.Contains(output, "Page ") {
		t.Fatalf("stdout unexpectedly contains pagination summary: %s", output)
	}
}
...
func TestSessionListAllJSONUsesAdminEndpoint(t *testing.T) {
	...
	cmd.SetOut(&stdout)
	cmd.SetErr(new(bytes.Buffer))
	cmd.SetArgs([]string{"session", "list", "--config-dir", configDir, "--all", "--output", "json"})
	...
	output := stdout.String()
	if !strings.Contains(output, `"username": "alice"`) || strings.HasPrefix(strings.TrimSpace(output), "{") {
