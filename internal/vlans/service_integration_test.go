...
func TestServiceCreateUpdateDeleteWritesDetailedAuditLogs(t *testing.T) {
	...
	updated, err := service.Update(ctx, actor, created.ID, api.VlanPatch{
		Name:        stringPtrIntegration("lab-updated"),
		VlanId:      int32PtrIntegration(31),
		Description: stringPtrIntegration("Updated lab devices"),
		IsActive:    boolPtrIntegration(false),
	})
	if err != nil {
		t.Fatalf("expected update to succeed, got %v", err)
	}

	if err := service.Delete(ctx, actor, updated.ID); err != nil {
...
func boolPtrIntegration(value bool) *bool {
	return &value
}
